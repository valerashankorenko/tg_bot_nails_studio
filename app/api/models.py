from sqlalchemy import BigInteger, Date, ForeignKey, Integer, String, Time
...
class User(Base):
    ...
    first_name: Mapped[str] = mapped_column(
        String, nullable=False)  # Имя пользователя
    username: Mapped[str] = mapped_column(
        String, nullable=True)  # Telegram username
...
class Master(Base):
    ...
    master_name: Mapped[str] = mapped_column(
        String, nullable=False)  # Имя мастера
...
class Service(Base):
    ...
    service_name: Mapped[str] = mapped_column(
        String, nullable=False)  # Название услуги
...
class Application(Base):
    ...
    user_id: Mapped[int] = mapped_column(BigInteger, ForeignKey(
        'users.telegram_id'))  # Внешний ключ на пользователя
    master_id: Mapped[int] = mapped_column(Integer, ForeignKey(
        'masters.master_id'))  # Внешний ключ на мастера
    service_id: Mapped[int] = mapped_column(Integer, ForeignKey(
        'services.service_id'))  # Внешний ключ на услугу
    appointment_date: Mapped[Date] = mapped_column(
        Date, nullable=False)  # Дата заявки
    appointment_time: Mapped[Time] = mapped_column(
        Time, nullable=False)  # Время заявки
